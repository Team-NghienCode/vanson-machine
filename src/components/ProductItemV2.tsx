...
export default function ProductItemV2(props: IProductItemV2Props) {
  ...
  return (
    <div
      className='group relative px-4 py-6 bg-gray rounded-md cursor-pointer'
      onClick={() => router.push('/san-pham/' + props.title.split(' ').join('-'))}>
          ...
          <div className='grid grid-cols-2 gap-2'>
            {props.sale && (
              <p className='text-md font-medium text-red-600 line-through'>
                {props.priceSale!.toLocaleString('de-DE')} ₫
              </p>
            )}
